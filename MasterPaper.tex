...
\section{紀要2019のはじめに}
社会におけるソフトウェアの重要性は高まっており、形式仕様記述言語を用いた仕様記述が重要視されてきている1)。
作成したソフトウェアにはテストが必要であるが、人手によるテストケースの設計には手間と時間がかかる。
そのため、VDM++仕様を対象としたテストケース自動生成を目的としたツールBWDM(Boundary Value/Vienna Develop Method) 2,3)を既に開発した。

既存のBWDMではVDM++仕様を対象として、境界値分析と記号実行を行い、境界値テストと、if-then-else式の構造認識に基づいたテストを実施するためのテストケースを自動生成する。
しかし、既存のBWDMは境界値分析時に全ての組合せを用いてテストケースを生成するため、組合せ爆発を起こす可能性がある。

組合せテストの総数を減らした上で、効果的なテストを行う方法としてペアワイズ法がある。
ソフトウェアについて、3つ以上の因子の組合せで発生する欠陥は、ほとんど存在しないことが知られている4)。
したがって、組合せテストは2つの因子の組合せに対して効果的である。
（3つ以上の因子で発生している欠陥を見つけるには、他の方法でテストする必要がある。）
そして、2つの因子のみの組合せをテストする手法のことをペアワイズ法と呼ぶ。

そこで本稿では、BWDMが組合せ爆発を起こす可能性の排除を目的として、BWDMによるテストケース生成にペアワイズ法を適用し、BWDMを拡張する。

本稿では、ペアワイズ法を適用するに当たって、Microsoft社が開発したPICT(Pairwise Independent Combinatorial Testing tool) 5)を利用する。
PICTはCLIツールであるが、API(PICTライブラリと呼称する)も提供しており、C++から利用できる。
しかし、既存のBWDMはJavaで記述していることから、PICTライブラリを呼び出すことができない。
そこで、PICTとBWDMを接続するためのインタフェースとして、PICTラッパーを開発した。
そして、既存のBWDMにPICTラッパーを埋め込むことで、BWDMを拡張した。

\section{SES2019のはじめに}
...
既存のBWDMは、１つの変数のみを含む条件式を持つ仕様の場合に、テストケースを生成できるが、
複数の変数を含む条件式を持つ仕様の場合は、テストケースを生成できない。

そこで、本研究では、既存のBWDMにおける、複数の変数を含む条件式を持つ仕様に対してテストケース生成ができないという問題点の解決を目的として、ドメイン分析テストのためのテストケース生成手法を提案し、BWDMに適用する拡張を行う。
...
\section{立山さん修論のはじめに}
コンピュータシステム上で動くソフトウェアが、
我々の社会において欠かすことのできないものとなり既に久しい。
ソフトウェアは年々大規模化かつ高機能化を続け、その結果、
ソフトウェアのバグが社会にもたらす影響も近年では甚大なものとなっている\cite{mizuho}。

ソフトウェアにバグが混入する原因の1つとして、
上流工程のソフトウェア設計段階において、自然言語を一般的に用いていることが挙げられる\cite{seminarFormalMethod}。
自然言語は元来、曖昧さを含んでいる。
そのため、プログラマが、仕様書上の表記を、仕様の作成者が本来意図していない意味で捉えてしまうことが起こる。
プログラマが、本来の仕様書の意図から外れた認識に基づいて実装を行った結果、
ソフトウェアにバグが混入されてしまう。

この問題を解決するための1つの方法として、
形式手法(Formal Methods)\cite{programSpecification}を用いた上流工程でのソフトウェア設計が挙げられる。
形式手法を用いた開発では、
数理論理学を基盤とした形式仕様記述言語(Formal Specification Language)により、
開発対象が持つ特性を仕様として記述する。
数理論理学を基にしているため、自然言語を用いた開発と異なり、
定理証明や機械的な検査を用いて、記述した内容が正しいことを数学的に証明することが可能である。
つまり、自然言語の持つ曖昧さを排除した、厳密な仕様を作成することが可能となる。

一方で、自然言語もしくは形式手法を用いた設計の、いずれにしても、
実装を行った後は、作成したソフトウェアに対してテストを行う必要がある。
テストを行うためには、テストケースの設計作業が必要であるが、
人手によるテストケースの設計には手間と時間がかかる\cite{standardtest}。
そのため、テストケースの設計を自動化することによって、テスト工程は効率化できる。

バグが潜みうる箇所に的を絞ってテストケース設計を行うことで、より少ないケース数で効率よくバグを見つけることができる。
これを可能とするテストケース設計技法として、一般的に、境界値分析\cite{bvaBlake, boundaryValueAnalysis}が知られている。

また、プログラム上の全ての実行フローを網羅するテストケース設計も重要である。
条件式による実行フローを解析する手法として、記号実行\cite{symbolic}が存在する。

そこで、本研究では、形式手法を用いたソフトウェア開発におけるテスト工程の作業効率化を目的として、
形式仕様を対象にした、境界値分析と記号実行の適用によるテストケース生成手法を提案する\cite{tachiyama1, tachiyama2, tachiyama3, tachiyama4, tachiyama5, tachiyama6, tachiyama7}。
具体的には、形式手法VDMで記述した関数定義中の、引数型とif-then-else式中の条件式(以後、if条件式)に対して境界値分析を行い、テストケース生成を行う。
加えて、記号実行によるテストケース生成も行う。

本研究で行う境界値分析は、if-then-else式がネストしていても、それらのif条件式同士の関係は考慮せずに、個々のif条件式を対象とする。
そのため、境界値分析だけで、実行フロー全てに対するテストケースを生成できない場合がある。
そこで、記号実行によるテストケース生成を並行して行う。
if-then-else式に対して記号実行を行い、全ての実行フローを解析することで、
境界値分析でカバーできなかった仕様中の箇所に対しても、テストケース生成が可能になる。

なお、境界値分析と記号実行は、それぞれ独立して、VDM++仕様に適用する。
つまり、本提案手法により最終的に生成するテストケースは、「境界値分析によるテストケース」と「記号実行によるテストケース」の2種類である。

また、本提案手法を基に、Java言語\cite{java}を用いて、テストケース自動生成ツールBWDM (Boundary Value/Vienna Development Method)を実装し、
本提案手法の実現性と実用性について評価する。
具体的には、実装したBWDMにVDM++仕様を適用した結果を示す。
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{出力例：テストケース\todo{出力例にペアワイズが入っているから直す。図ではなくてコードにする。}}
...
図\ref{fig:testcase_sample}において、“各引数の境界値”には、引数a、b、cそれぞれの境界値の集合を出力している。
