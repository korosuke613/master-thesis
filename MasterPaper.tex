...
プログラマが、本来の仕様書の意図から外れた認識に基づいて実装を行った結果、ソフトウェアにバグが混入されてしまう。
...
そのため、VDM++仕様を対象としたテストケース自動生成を目的としたツールBWDM（Boundary Value/Vienna Develop Method）を立山らが開発した\cite{tachiyama1, tachiyama2,tachiyama3,tachiyama4, tachiyama5, tachiyama6, tachiyama7, tachiyama8}。
...
\begin{enumerate}[label=(\alph*)]
  \item\label{problem:1} 境界値分析時にすべての組合せを用いてテストケースを生成するため、組合せ爆発を起こす可能性があるという問題
...
そこで、本研究では、上記5つの問題点を解決することを目的として、BWDMの拡張を行う\cite{hirakoba1, hirakoba2, hirakoba3, hirakoba4, hirakoba5}。
...
\section{VDM++}
形式手法の1つにVDM(Vienna Development Method)がある。
...
また、本研究では、Overture Projectが発行しているVDM-10 Language Manualで定義されているVDM++を対称とする\cite{vdm_manual}。

VDMには、VDMTools\cite{vdmtools}などの支援ツールが揃っており、
仕様の検証などを他の形式手法に比べ比較的行いやすいという利点がある。

VDM++では、関数や操作の定義を定義ブロック(\ttt{definition block})で行う。
それぞれの定義と、各定義に対応する日本語での呼び方を表\ref{tab:vdm_definition_japanese}に示す。
\begin{table}[t]
  \begin{center}
...
また、本研究で拡張するBWDMは、あらたに、表\ref{tab:vdm_definition_japanese}における、定数定義、操作定義、インスタンス変数定義に対応する。
詳細を、以下に示す。
...
\begin{itemize}
  \item 定数定義\\
        ...
        ただし、定義する定数の型は\ttt{int}、\ttt{nat}、\ttt{nat1}のいずれかでなければならない。
  \item 操作定義\\
        操作定義は、Javaにおけるメソッドを表す。
        定義した操作は、同クラス内のインスタンス変数を参照できる。
        拡張するBWDMは、操作定義に対応することで、if-then-else式の構造認識に基づいたテストを実施するためのテストケースを自動生成する。
        ただし、定義する操作内では\ttt{int}、\ttt{nat}、\ttt{nat1}と\ttt{sec of char}(戻り値のみ)しか利用できない。
        また、事前条件、事後条件は対応しない。
  \item インスタンス変数定義\\
        インスタンス変数定義は、Javaにおけるフィールドを表す。
        定義した定数は、関数定義、操作定義で利用できる。
        拡張するBWDMは、インスタンス変数定義に対応することで、操作定義内でインスタンス変数を利用できる。
        ただし、定義するインスタンス変数の型は\ttt{int}、\ttt{nat}、\ttt{nat1}のいずれかでなければならない。
\end{itemize}
...
\section{PICT(Pairwise Independent Combinatorial Testing)}
\todo{pictの説明を書く}
...
\todo{因子と水準の定義で引っ張ってこれる文献がネットで見つけられない。
  みな当たり前のように因子と水準という言葉を使っている。
  実験計画法の本に載ってそうだが、8000円くらいする。}
...
例えば、性別$\{"男性", "女性", "その他"\}$と年齢$\{"20歳未満", "20歳以上"\}$を入力として出力が決定される関数の場合、因子と水準は表\ref{tab:factor_level}となる。
...
\todo{一章にも同じようなことが書いている。両方残すか、片方だけ残すか}
ペアワイズ法とは、組合せテストの総数を減らした上で、効果的なテストを行う方法である\cite{pairwise}。
ソフトウェアについて、3つ以上の因子の組合せで発生する欠陥は、ほとんど存在しないことが知られている\cite{over3fact}。
したがって、組合せテストは2つの因子の組合せに対して効果的である。
（3つ以上の因子で発生している欠陥を見つけるには、他の方法でテストする必要がある。）
...
ペアワイズ法の例を示す。
\ref{sec:factor_and_level}節の例の関数には、2つの因子が存在したが、あらたに、国籍$\{"日本", "アメリカ", "その他"\}$を追加する。
あらたに定義した関数は、因子3、水準が(3, 2, 3)である。
この関数の総組合せでテストケースを作成した場合、総数は$3 \times 2 \times 3 = 18個$となる。
しかし、ペアワイズ法を用いてテストケースを作成した場合、総数を9個程度(表\ref{tab:example_pairwise}参照)に抑えることができる。
...
\begin{table}[tp]
  \begin{center}
    ...
    \begin{tabular}{c|c}
      ...
      複数の定義への対応         &
      \begin{tabular}{c}
        単一の関数しか                         \\テストケースを生成できない問題点                       \\(\ref{sec:probrem_only_function}節参照) \\ \hline
        定数定義ブロック内の定義を含む関数の   \\テストケースを生成できない問題点 \\(\ref{sec:probrem_value}節参照) \\ \hline
...
VDM++仕様を対象としたテストケース自動生成を目的としたツールBWDM（Boundary Value/Vienna Develop Method）は、VDM++仕様を入力として、境界値分析と記号実行を行い、境界値テストと、if-then-else式の構造認識に基づいたテストを実施するためのテストケースを自動生成する。

しかし、既存のBWDMには5つの問題点がある。
問題点を下記に示す。

\begin{enumerate}[label=(\alph*)]
  \item 境界値分析時に全ての組合せを用いてテストケースを生成するため、組合せ爆発を起こす可能性があるという問題
  \item 複数の変数を含む条件式を持つ関数のテストケースを生成できない問題
  \item 単一の関数しかテストケースを生成できないという問題
  \item 定数定義ブロック内の定義を含む関数を含む仕様のテストケースを生成できないという問題
  \item 操作のテストケースを生成できないという問題
\end{enumerate}

上記5つの、問題点を解決することを目的として、既存のBWDMに、ペアワイズ法とドメイン分析テストの適用のための機能拡張を行った。
\todo{残った、複数定義への対応の拡張はどう入れるか}
...
\todo{考察を元に、複数定義への対応に関するまとめを書く}

以上により、本研究で行った拡張によって、
既存のBWDMの5つの問題点を解決し、BWDMの有用性が向上したと言える。
\todo{BWDMのテストケース生成効率が向上したと言える理由を書く。}また、BWDMのテストケース生成効率が向上したと言える。
...
\begin{itemize}
  \item 整数型以外の型への対応
        ...
        現状、実数を表すreal型や、有理数を表すrat型、複数の型から構成する合成型などの、VDMの多くの型に未対応である。

        VDM++仕様を静的解析する際に型情報を読み込み、
        境界値分析時に、読み込んだ型情報から境界値の生成処理をBWDMに追加することによって、対応可能であると考えられる。

  \item 関数定義以外の定義部への対応

        現状、VDM++仕様中の関数定義部のみから境界値の抽出を行っている。

        操作定義部や型定義部などの、仕様上の他の定義部に対しても静的解析を行い、情報を抽出し、
        それらの定義に対する新たな境界値分析手法を考案することで、
        対応が可能であると考えている。

  \item 両辺が変数であるif条件式への対応

        現状の手法では、if条件式中の変数を1つに制限している。

        変数が2つ以上の場合の、境界値分析を行う手法を考案する必要がある。
\end{itemize}
