...
\todo{終わりにをかく}
\section{紀要2019のおわりに}
本稿では、VDM++仕様を対象とする境界値分析を基にしたテストケース自動生成ツールBWDMにおける、境界値分析結果から生成するテストケース数が組合せ爆発を起こす可能性の排除を目的として、BWDMの拡張を行った。
...
さらに、拡張後のBWDMは実用性が高いと言える。


\section{ses2019のおわりに}
...
また、VDM++仕様からドメイン分析テストのためのテストケースを自動生成することが可能となったため、テストケース生成効率が向上したと言える。

\section{立山さん修論のおわりに}
本研究では、形式手法を用いたソフトウェア開発におけるテスト工程の作業効率化を目的として、
形式仕様を対象にした、境界値分析と記号実行の適用によるテストケース生成手法を提案した。

提案手法では、形式手法VDMによる仕様に対して、境界値分析と記号実行を個別に適用した。
まず、境界値分析をVDM++による仕様へ適用し、実装後にプログラム上のバグが入り込みやすい箇所への効率的なテストを可能にするテストケースを生成した。
そして、記号実行をVDM++仕様へ適用し、関数定義上の実行フローの全てに対するテストケースを生成した。
これにより、境界値分析で実行フローを網羅できなかった場合でも、プログラムに対するテストの漏れを防ぐことができる。

提案手法を基に、テストケース生成ツールBWDMの実装を行い、その詳細を示した。
BWDMは、VDM支援ツールVDMJとSMTソルバZ3を利用し、境界値分析と記号実行によるテストケース生成を自動で行う。

適用例として、BWDMに対して、「ネストを用いた仕様」、「剰余式と不等式が混在した仕様」、「境界値分析で網羅できない実行フローを持つ仕様」の
3つのVDM++仕様を適用した。
その結果、提案手法によって、
VDM++仕様から境界値によるテストケースと、記号実行によるテストケースを生成できることを確認した。
また、有用性の評価のために、if条件式をそれぞれ5個、10個、15個持つVDM++仕様をBWDMに適用し、その実行に要した時間を計測した。
結果、全ての仕様に対して、それぞれ1秒未満の時間で、テストケース生成処理が可能なことを確認できた。

考察では、人手によるテストケース生成との、要した時間および生成したテストケースの内容について比較検証を行った。
人手による生成は、10分前後の時間を要した上に、ヒューマンエラーによって、誤ったテストケースを含む場合があった。
これに対して、BWDMは1秒未満の時間で、問題なく生成を終えた。

以上のことから、形式手法を用いたソフトウェア開発において、本提案手法を用いることにより、
テスト工程の作業効率化が見込まれる。
