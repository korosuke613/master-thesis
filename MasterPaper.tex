...
拡張したBWDMでは、複数の関数のテストケースを生成できるようになり(\ref{sec:tekiyou_multi_func}節参照)、また、定数定義ブロックで定義した定数を、関数定義や操作定義内でを参照できるようになった(\ref{sec:tekiyou_value}節参照)。
...
上記5つの問題点を解決するために、既存のBWDMに、ペアワイズ法とドメイン分析テストの適用のための3つの機能拡張を行った。
