...
\todo{秋山さんのソフトウェアテスト技法ドリルには、
〜〜「画面解像度」「ブラウザ拡大率」「ブラウザの文字サイズ」といった機能を因子、
それぞれの機能に含まれている「1024と1920」「100\%と125\%」「中と大」といった選択肢を水準と呼びます。〜〜
と書かれており、やはり定義はありませんでした。(93ページ)}
...
ただし、拡張するBWDMは、if-then-else構文を含まない操作である「西暦設定」操作をテストケース生成の対象としない。
...
\begin{itemize}
  ...
  \item 関数定義と操作定義内で利用できる構文の対応範囲拡大
        ...
        対応していない構文の情報抽出方法と、境界値分析手法の提案を行うことで対応可能と考える。
