  ...
  \begin{center}
    ...
    \begin{tabular}{c|c}
      ...
      \end{tabular} 
                                 &
      \begin{tabular}{c}
        組合せ爆発に関する問題点 \\(\ref{sec:problem_pairwise}節)
      \end{tabular}                \\ \hline
      \begin{tabular}{c}
        ドメイン分析テストの適用による \\複数変数を含む条件式を含む関数の\\テストケース生成
      \end{tabular} &
      \begin{tabular}{c}
        複数変数を含む条件式に関する問題点 \\(\ref{sec:problem_double_var}節)
      \end{tabular}                \\ \hline
      複数の定義への対応         &
      \begin{tabular}{c}
        単一の関数しか\\テストケースを生成できない問題点                       \\(\ref{sec:probrem_only_function}節)               \\
        定数定義ブロック内の定義を含む関数の\\テストケースを生成できない問題点 \\(\ref{sec:probrem_value}節) \\
        操作のテストケースを生成できない問題点                               \\(\ref{sec:probrem_operation}節)
      \end{tabular}
    \end{tabular}
  \end{center}
\end{table}
