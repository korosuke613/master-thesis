...
まず、既存のBWDMにおける、境界値分析結果から生成するテストケース数が組合せ爆発を起こす可能性の排除を目的として、BWDMの拡張を行う。
...
次に、本研究では、既存のBWDMにおける、複数の変数を含む条件式を持つ仕様に対してテストケース生成ができないという問題の解決を目的として、ドメイン分析テストのためのテストケース生成を、BWDMに適用する拡張う。
まず、複数の変数を含んだ条件式を解析できるように、構文解析部を拡張する。
次に、ドメイン分析部を追加し、onポイント、offポイント、inポイント、outポイントを生成できるようにする。
...
また、BWDMがテストケースを自動生成できるVDM++仕様の構文が増えたことにより、テスト効率が向上したと言える。
...
具体的には、既存のBWDMがもつ、上記5つの問題点を解決する。
また、拡張後のBWDMの名称を、\tool{}(\toolFullName{})に変更する。
...
\begin{itemize}
  ...
  \item $condition$が$exp >= int$のとき、$TB = int$とする。
\end{itemize}
$condition$を条件式、$exp$を左辺、$int$を右辺とし、
...
コード\ref{fig:extended_multiple_definition}では、インスタン��変数「current\_year」を「うるう年判定」操作の引数としてテストケース生成できていることが確認できる。
