...
これらの問題を解決するために、拡張した\tool{}では、大きく分けて以下の3つの拡張を行う。
...
\begin{table}[t]
  \begin{center}
    ...
    \begin{tabular}{c|c}
      ...
      \end{tabular} & 組合せ爆発に関する問題点           \\
      \begin{tabular}{c}
        ドメイン分析テストの適用による \\複数変数を含む条件式を含む関数の\\テストケース生成
      \end{tabular} & 複数変数を含む条件式に関する問題点 \\
      複数の定義への対応         &
      \begin{tabular}{c}
        単一の関数しかテストケース生成できない問題点                         \\
        定数定義ブロック内の定義を含む関数のテストケースを生成できない問題点 \\
        操作のテストケースを生成できない問題点
      \end{tabular}                                      \\
      \hline
    \end{tabular}
