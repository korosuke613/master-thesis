...
\lstset{language=}
\begin{figure}[tp]
  \begin{lstlisting}[caption={因子が3で境界値生成後の水準が(6, 6, 6)の関数を持つVDM++仕様},label=fig:pict4javaSampleVdm]
class SampleClass
functions
...
\lstset{language=}
\begin{figure}[tp]
  \begin{lstlisting}[caption={拡張したBWDMに因子が3で境界値生成後の水準が(6, 6, 6)の関数を持つ仕様(コード\ref{fig:pict4javaSampleVdm})を適用した際の出力},label=fig:pict4javaTekiyourei]
