...
例えば、性別$\{"男性", "女性", "その他"\}$と年齢$\{"20歳未満", "20歳以上"\}$を入力として出力が決定される関数の場合の因子と水準を表\ref{tab:factor_level}に示す。
...
ペアワイズ法の例を示す。
\ref{sec:factor_and_level}節の例の関数には、2つの因子が存在したが、新たに、国籍$\{"日本", "アメリカ", "その他"\}$を追加する。
新たに定義した関数は、因子3、水準が(3, 2, 3)である。
